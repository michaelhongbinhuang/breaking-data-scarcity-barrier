...
#######################################
# Convert datatype from prices series #
#######################################

def convert_datatype(timeseries, datatype):
  """ Convert the datatype of prices series """
  if datatype == 'prices':
    timeseries = timeseries
  elif datatype == 'returns':
    timeseries = timeseries[1:] / timeseries[:-1] - 1
  else:
    raise AttributeError(f"Datatype {datatype} is undefined.")
  return timeseries
...
def returns2prices_mat(returns, start=1):
  """ Construct prices series from returns series (for a numpy array) """
  prices = returns2prices(returns.flatten(), start)
  return prices.reshape(returns.shape)
